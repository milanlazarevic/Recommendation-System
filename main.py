...
def main():
    ...
    df_ratings = parse_csv(RATINGS_SMALL)
    df_ratings = df_ratings[df_ratings["movieId"].isin(df_movies["id"])]

    print(df_ratings[df_ratings['userId'] == 1].head(10))

    cbf = ContentBasedFilter(df_movies, df_ratings)
    cbf.calculate_scores()

    print(cbf.get_recommendation_for_user(2, 10)['title'].head(10))
